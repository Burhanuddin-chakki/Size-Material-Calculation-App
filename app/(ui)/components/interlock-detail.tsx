...
export const interLockPipeSchema = z.object({
  ...
  smallInterLockWeight: z
    .number()
    .min(1, "InterLock weight must be greater than 0"),
  bigInterLockWeight: z
    .number()
    .min(1, "InterLock weight must be greater than 0"),
  interLockPipeSize180: z.boolean(),
  interLockPipeSize192: z.boolean(),
  interLockExtraLength: z
...
export default function InterLockDetail(props: InterLockDetailProps) {
  ...
  useEffect(() => {
    const interLockPipeDetail = props.pipeDetail.find(
      (pd) => pd.pipeType === "InterLock",
    );
    setValue(
      "smallInterLockWeight",
      interLockPipeDetail?.pipeSizes[0].weight || 0,
    );
    setValue(
      "bigInterLockWeight",
