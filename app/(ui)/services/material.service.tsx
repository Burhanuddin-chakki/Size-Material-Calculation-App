...
export const handleMaterialFunction = (field: string, inputData: any): MaterialEstimationResult => {
    ...
    return functionMapping[field] ? functionMapping[field](inputData) : { quantity: 0, rate: 0, totalPrice: 0 };
}

export const getNoOfPannels = (inputData: any): number => inputData.numberOfDoors === inputData.numberOfTrack ? inputData.numberOfDoors : inputData.numberOfDoors > inputData.numberOfTrack ? inputData.numberOfDoors : inputData.numberOfTrack;
...
const getLockQuantity = (inputData: any): MaterialEstimationResult => {
    const quantity = inputData.isContainMacharJali ? getNoOfPannels(inputData) : getNoOfPannels(inputData) - 1;
...
const getScrew13_6Quantity = (inputData: any): MaterialEstimationResult => {
    ...
    if (inputData.selectedSpOrDpPipe === "DP" || inputData.selectedSpOrDpPipe === "SP") {
        quantity += 16;
    }
...
const getLabourQuantity = (inputData: any): MaterialEstimationResult => {
    const quantity = getRoundFeet(inputData.height) * getRoundFeet(inputData.width)
...
const getSiliconQuantity = (inputData: any): MaterialEstimationResult => {
    return { quantity: 0.5, rate: inputData.silicon, totalPrice: 0.5 * inputData.silicon };
}
const getScrew75_10Quantity = (inputData: any): MaterialEstimationResult => {
    return { quantity: 6, rate: inputData.screw75_10, totalPrice: 6 * inputData.screw75_10 };
}
const getScrew25_6Quantity = (inputData: any): MaterialEstimationResult => {
    return { quantity: 6, rate: inputData.screw25_6, totalPrice: 6 * inputData.screw25_6 };
}
...
const getMacharJaliQuantity = (inputData: any): { quantity: number, rate: number } => {
    ...
    const selectedPannelSizeWithWidth = selectJaliPannel(partitionWidth, panelSizes);
    const selectedPannelSizeWithHeight = selectJaliPannel(inputData.height, panelSizes);
    const selectedPannelSize = Math.min(selectedPannelSizeWithWidth, selectedPannelSizeWithHeight);
    const quantity = selectedPannelSize * getRoundFeet(inputData.height)
    const rate = quantity * inputData.macharJali;
    return { quantity, rate };
}
const getGrillJaliQuantity = (inputData: any): { quantity: number, rate: number } => {
    const partitionWidth = inputData.width / inputData.numberOfDoors;
