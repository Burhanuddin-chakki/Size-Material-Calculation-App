...
const MaterialPrice = dynamic(() => import("@/app/(ui)/components/material-price"), {
  loading: () => <div className="text-center p-3"><div className="spinner-border spinner-border-sm" role="status"></div></div>
});

const EstimationDetail = dynamic(() => import("@/app/(ui)/components/estimation-detail"), {
  loading: () => (
    ...
    const pipeDetailComponents = useMemo(() => {
        if (!isLoading && windowType && windowType in pipeTypeToComponentMapping) {
            ...
            return components.map((Component, index) => (
                <div className="row main-container" style={{ marginTop: "1.5rem" }} key={`component-${index}`}>
                    <Component pipeType={pipeType} pipeDetail={pipeDetail} />
                </div>
            ));
        }
        return [];
    }, [isLoading, windowType, pipeType, pipeDetail, selectedSpOrDpPipe, setIncludeSpDpSchema]);
    ...
    return (
        <>
            <div className="window-fixed-top text-center">
                {showEstimationDetailView && <BackButton onClick={() => setShowEstimationDetailView(false)} position="start-0 ms-3" buttonName="Back" />}
                <h4 className="mb-0">{windowType}</h4>
                <BackButton onClick={() => toggleZoom()} position="end-0 me-3" buttonName={isZoomedOut ? "100%" : "75%"} />
            </div>
