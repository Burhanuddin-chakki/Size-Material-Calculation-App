...
import { MaterialType, PipeDetailType, PipeType, WindowType } from "../common/interfaces";
...
export async function fetchMaterialList(windowId: number): Promise<MaterialType[]> { 
    await connectDb();
    const response = await MaterialModel.find({windowTypeId: windowId}, { _id: 0 }).sort({ id: 1 }).lean();
    return JSON.parse(JSON.stringify(response));
}
...
export async function fetchPipeDetail(windowId: number): Promise<PipeDetailType[]> {
    await connectDb();
    const response = await PipeModel.find({ windowType: windowId }, { _id: 0, id: 1, pipeName: 1, pipeType: 1, pipeSizeUnit: 1, pipeWeightUnit: 1, windowType: 1, pipeSizes: 1 }).sort({ id: 1 }).lean();
    return JSON.parse(JSON.stringify(response));
}